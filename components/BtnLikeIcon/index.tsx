...
const BtnLikeIcon: FC<BtnLikeIconProps> = ({
    ...
    const handleLike = (state:any) => {
      setLikedState(state);
      const localLike = localStorage.getItem(idProperty);
      if (localLike) {
          localStorage.removeItem(idProperty.toString());
          return;
      }
        localStorage.setItem(idProperty.toString(), idProperty);
    };
