...
const NavigationItem: FC<any> = ({
                    ...
                    {menu.megaMenu?.map((item) => (
                      <div key={item.id}>
                        ...
                        <ul className="grid space-y-1">
                          {item.items.map(renderMegaMenuLink)}
                        </ul>
                      </div>
                    ))}
                  </div>
  ...
  const renderMegaMenuLink = (item: NavItemType) => {
    return (
      <li key={item.id}>
        <Link
              ...
              >
                <ul className="relative grid py-4 space-y-1 text-sm bg-white rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-white dark:ring-opacity-10 dark:bg-neutral-900">
                  {menuDropdown.children?.map((i) => {
                    if (i.type) {
                      return renderDropdownMenuLinkHasChild(i);
                    } else {
                      return (
                        <li
                          key={i.id}
  ...
  const renderDropdownMenuLinkHasChild = (item: NavItemType) => {
  ...
  const renderDropdownMenuLink = (item: NavItemType) => {
    return (
        <Link
            target={item.targetBlank ? "_blank" : undefined}
            rel="noopener noreferrer"
